...
void AsyncSwitchSend(int target_node_id, Path& selected_path, int &reduce_id, std::map<int, int>all_reduce_descriptor, std::map<int, Host>& host_map, std::map<int, Switch>& switch_map) {
    ...
    if (selected_path.upper_node[0] == 'S'){
        ...
        target.receive(reduce_id, all_reduce_descriptor[reduce_id], host_map, switch_map);
    }else{
...
void Switch::send(int reduce_id, int data, std::map<int, Host> &host_map, std::map<int, Switch> &switch_map){
    ...
    executions.emplace_back(AsyncSwitchSend, target_node_id, std::ref(selected_path), std::ref(reduce_id), std::ref(all_reduce_descriptor), std::ref(host_map), std::ref(switch_map));
}
